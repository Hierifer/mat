export default {
  settings: {
    title: '偏好设置',
    appearance: '外观',
    general: '通用',
    view: '视图',
    shortcuts: '快捷键',
    colorTheme: '配色主题',
    windowAppearance: '窗口外观',
    dimInactivePanes: '未聚焦窗格变灰',
    dimInactivePanesDesc: '对未聚焦的终端窗格应用灰度效果',
    inactiveBrightness: '未聚焦窗格亮度',
    inactiveBrightnessDesc: '数值越低，未聚焦窗格越暗',
    notifications: '通知',
    enableCommandNotifications: '启用命令完成通知',
    enableCommandNotificationsDesc: '当 Claude 命令执行完成时发送系统通知',
    language: '语言',
    displayMode: '布局模式',
    displayModeTabs: '标签模式',
    displayModeStudio: 'Studio 模式',
    displayModeDesc: '切换标签页和 Studio 布局',
    fontSize: '字体大小',
    fontSizeDesc: '终端文字大小',
    themeMode: '主题模式',
    themeModeAuto: '自动（跟随系统）',
    themeModeLight: '浅色',
    themeModeDark: '深色',
    tmux: {
      title: 'tmux 集成',
      enable: '启用 tmux 会话管理',
      enableDesc: '使用 tmux 实现会话持久化和无限回滚，重启应用时自动恢复之前的终端布局与会话',
      manageSession: '管理 tmux 会话',
    },
    speech: {
      title: '语音识别',
      provider: '识别引擎',
      whisperLocal: 'Whisper（本地）',
      alibabaCloud: '阿里云智能语音',
      alibabaApiKey: 'API Key',
      saveCredentials: '保存',
      credentialsSaved: '已保存',
      alibabaDesc: '使用百炼平台 Paraformer 语音识别服务，需在百炼控制台获取 API Key',
    },
    tts: {
      title: '语音播报',
      enable: '启用语音播报',
      enableDesc: '当 Claude Code 完成任务或等待输入时通过语音提醒',
      voice: '播报音色',
      noApiKey: '请先在上方语音识别设置中配置阿里云 API Key',
    },
  },
  sessionManager: {
    title: 'tmux 会话管理器',
    attach: '附加',
    detach: '分离',
    rename: '重命名',
    delete: '删除',
    newSession: '新建会话',
    newSessionPlaceholder: '会话名称...',
    newSessionName: '新会话名称',
    refresh: '刷新',
    confirmDelete: '确定要删除会话 {name} 吗？这将终止所有相关进程。',
    noSessions: '未找到 tmux 会话',
    windows: '个窗口',
  },
  terminal: {
    tmuxMode: 'tmux 模式',
    loadMoreHistory: '加载更多历史记录',
    noSessions: '无终端会话',
  },
  menu: {
    about: '关于 Materm',
    checkUpdates: '检查更新...',
    settings: '设置...',
    closeTab: '关闭标签页',
    closeWindow: '关闭窗口',
    quit: '退出',
    copy: '拷贝',
    paste: '粘贴',
    selectAll: '全选',
    clearTerminal: '清空终端',
    splitHorizontally: '水平分屏',
    splitVertically: '垂直分屏',
    zoomIn: '放大',
    zoomOut: '缩小',
    resetZoom: '重置缩放',
    toggleFullscreen: '切换全屏',
  },
  notifications: {
    taskComplete: '任务完成',
    updateAvailable: '发现新版本',
    updateAvailableDesc: '点击更新对话框查看详情',
    updateAvailableStartup: '有新版本可用，点击查看更新详情',
    alreadyLatest: '已是最新版本',
    alreadyLatestDesc: '您正在使用最新版本的 Materm',
    claudeTaskComplete: 'Claude 任务完成',
    commandCompleted: '命令执行完成',
  },
  updater: {
    title: '🎉 新版本可用',
    checkingTitle: '检查更新',
    checkingDesc: '正在检查新版本...',
    noUpdateTitle: '已是最新版本',
    noUpdateDesc: '当前已是最新版本，无需更新。',
    version: '版本',
    releaseNotes: '更新内容',
    updateFailed: '更新失败：{error}',
    downloading: '下载中...',
    downloadProgress: '下载中... {progress}%',
    remindLater: '稍后提醒',
    dismissVersion: '不再提醒此版本',
    updateNow: '立即更新',
    restartNow: '立即重启',
    readyToRestart: '更新已安装。重启以应用新版本。',
  },
  whatsNew: {
    title: '更新内容',
    features: '新功能',
    fixes: '问题修复',
  },
  about: {
    description: '现代终端模拟器，支持 iTerm2 风格特性',
    featuresTitle: '功能特性',
    features: {
      splitPanes: '水平和垂直分割窗格',
      multipleTabs: '多标签页支持',
      customThemes: '可自定义主题',
      nativePerf: '基于 Tauri 的原生性能',
    },
    creditsTitle: '致谢',
    builtWith: '使用以下技术构建：',
    copyright: '© {year} Hierifer. 保留所有权利。',
  },
  layout: {
    presets: '预制布局',
    dual: '左右双栏',
    triple: '左中右三栏',
    quad: '四宫格',
  },
  search: {
    placeholder: '搜索关键词...',
    noResults: '未找到匹配',
    matchCount: '{current}/{total}',
  },
  paneToolbar: {
    splitHorizontal: '水平分割',
    splitVertical: '垂直分割',
    closePane: '关闭窗格',
  },
  app: {
    alreadyLatest: '您已经在使用最新版本！',
    checkUpdateFailed: '检查更新失败：{error}',
  },
  studio: {
    title: 'Studio 模式',
    selectProject: '选择项目',
    selectProjectDesc: '选择一个 Git 仓库文件夹',
    notGitRepo: '非 Git 仓库',
    initGitConfirm: '所选文件夹不是 Git 仓库，是否为您执行 git init 初始化？',
    newBranch: '新建分支',
    branchName: '分支名',
    branchNamePlaceholder: 'feat/my-feature...',
    deleteBranch: '删除分支',
    confirmDelete: '确定删除分支 "{name}"？这将移除工作树并删除分支。',
    defaultBranch: '默认分支',
    addProject: '添加项目',
    projects: '项目',
    recentProjects: '最近项目',
    noRecentProjects: '暂无最近项目',
    openProject: '打开项目...',
    removeFromRecent: '从列表移除',
    exitStudio: '退出 Studio',
    switchToTabs: '切换到标签页',
    agent: {
      agentView: 'Agent',
      terminalView: '终端',
      inputPlaceholder: '向 Claude Code 发送消息，Enter 发送，Shift+Enter 换行',
      send: '发送',
      ready: '就绪',
      thinking: '思考中...',
      restart: '重启会话',
      emptyHint: '输入任务，Claude Code 将在此分支的工作树中执行',
      sessionExited: '会话已结束，点击重启按钮开始新会话',
    },
    gitPanel: {
      status: '变更',
      staged: '已暂存',
      modified: '已修改',
      untracked: '未跟踪',
      noChanges: '工作区干净',
      stash: '储藏',
      stashSave: '保存',
      stashMessagePlaceholder: '储藏说明...',
      stashIncludeUntracked: '包含未跟踪文件',
      stashPop: '弹出',
      stashApply: '应用',
      stashDrop: '删除',
      stashEmpty: '无储藏',
      stashConfirmDrop: '确定要删除此储藏吗？',
      log: '提交',
      logEmpty: '无提交记录',
      refresh: '刷新',
    },
  },
  common: {
    close: '关闭',
    cancel: '取消',
    ok: '确定',
    confirm: '确认',
    apply: '应用',
    reset: '重置',
    justNow: '刚刚',
    minutesAgo: '{minutes}分钟前',
    hoursAgo: '{hours}小时前',
    daysAgo: '{days}天前',
  }
}
